...
public class ConfermaOrdine extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            // Nota: Se hai tolto il vincolo FK, questa riga non darà più errore anche se l'email non è in newsletter
            ...
            // --- PARTE MODIFICATA: CONTROLLO SCONTO NEWSLETTER ---
            
            // Verifichiamo se l'email è presente nella tabella newsletter
            String sqlSconto = "SELECT 1 FROM newsletter WHERE email = ?";
            PreparedStatement psSconto = conn.prepareStatement(sqlSconto);
            psSconto.setString(1, emailCliente);
            ResultSet rsSconto = psSconto.executeQuery();
            
            if (rsSconto.next()) {
                // Se l'email esiste, applichiamo lo sconto del 10%
                totaleFinale = totaleFinale * 0.90;
                System.out.println("DEBUG: Sconto 10% applicato per " + emailCliente);
            }
            
            // ----------------------------------------------------

            // 4. Aggiornamento dell'ordine con il totale calcolato (scontato o meno)
            ...
            response.sendRedirect("index.html?ordine=successo"); 
